...
import { type KanbanColumn, COLUMN_TO_STATUS, COLUMN_FOLDERS } from "./types";
...
export class JiraMover implements WorkItemMover {
  ...
  async move(file: TFile, targetColumnId: string): Promise<boolean> {
    ...
    let updated = content;
    updated = updated.replace(/^_pending-transition:\s*.+$/m, `_pending-transition: true`);
    ...
    try {
      ...
      if (!transition) {
        ...
        reverted = reverted.replace(/^_pending-transition:\s*.+$/m, `_pending-transition: false`);
        reverted = reverted.replace(/^state:\s*.+$/m, `state: ${this.columnFromPath(file.path)}`);
        await this.app.vault.modify(file, reverted);

        const available = transitions.map((t: any) => t.to.name).join(", ");
        console.warn(
          `[jira-terminal] Cannot transition ${jiraKey} to "${targetStatus}". ` +
      ...
      transitioned = transitioned.replace(/^jira-status:\s*.+$/m, `jira-status: "${targetStatus}"`);
      transitioned = transitioned.replace(
        /^_pending-transition:\s*.+$/m,
        `_pending-transition: false`,
      );
      const now = new Date().toISOString().replace(/\.\d{3}Z$/, "Z");
      transitioned = transitioned.replace(/^updated:\s*.+$/m, `updated: ${now}`);
      ...
      if (file.path !== newPath) {
        const folder = this.app.vault.getAbstractFileByPath(`${this.basePath}/${targetFolder}`);
        if (!folder) {
